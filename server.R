...
shinyServer(function(input, output,session) {
  ...
  numoftweets<-function(entity1,entity1entry){
    ent1numtweets<-nrow(entity1)
    notweets<-ent1numtweets
    names(notweets)<-entity1entry
    notweets
  } 
  ...
  sentimentalanalysis<-function(entity1text,entity1entry){
    ...
    # Movie Rating
    points<-entity1score[,'score']
    m<-mean(points)
    n<-m-min(points)
    range<-max(points)-min(points)
    rating<-(n*5)/range
    round(rating,digits = 1)
  ...
  entity1score<-reactive({
    ...
    entity1score<-sentimentalanalysis(entity1()$text,input$entity1)})
  ...
  output$notweets<-renderPrint({
    ...
    numoftweets(entity1(),input$entity1)})
  ...
  output$sentiboxplot<-renderPlot({
    ...
    sentiboxplot<-ggplot(entity1score(),aes(x=size,y=score))+
      facet_grid(entity ~ .)+
  ...
  output$sentiheadtable<-renderTable({tab<-head(entity1score(),100)})
  output$sentitailtable<-renderTable({tab<-tail(entity1score(),100)})
